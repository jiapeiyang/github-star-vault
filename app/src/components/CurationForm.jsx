...
export function CurationForm({ repo, catalog, onOpenRepo, onDirtyChange }) {
  ...
  return <form className="curation-form" noValidate onSubmit={(event) => { event.preventDefault(); exportFile("download"); }}>
    ...
    <div className="curation-grid">
      ...
      <label className="full-field"><span>仓库摘要</span><textarea aria-label="仓库摘要" value={form.summary} onChange={(event) => change({ summary: event.target.value })} rows={3} placeholder="这个仓库解决什么问题？" {...fieldProps("summary")} />{error("summary")}</label>
      <fieldset className="tag-editor"><legend>策展标签 · 最多 5 个</legend>{form.tags.map((tag, index) => <div key={index}><input aria-label={`标签 ${index + 1}`} value={tag} onChange={(event) => change({ tags: form.tags.map((value, i) => i === index ? event.target.value : value) })} {...fieldProps("tags")} /><button type="button" aria-label={`移除标签 ${index + 1}`} onClick={() => change({ tags: form.tags.filter((_, i) => i !== index) })}><X size={16} /></button></div>)}<button type="button" className="text-link" disabled={form.tags.length >= 5} onClick={() => change({ tags: [...form.tags, ""] })}><Plus size={16} /> 添加标签</button>{error("tags")}</fieldset>
      ...
      <div className="full-field content-editor"><div className="content-editor-tabs" role="group" aria-label="资料显示方式"><button type="button" aria-pressed={!preview} onClick={() => setPreview(false)}>编辑正文</button><button type="button" aria-pressed={preview} onClick={() => setPreview(true)}>阅读预览</button></div>{preview ? <MarkdownContent>{form.body}</MarkdownContent> : <label><span>仓库解读 · Markdown</span><textarea value={form.body} onChange={(event) => change({ body: event.target.value })} rows={15} spellCheck={false} {...fieldProps("body")} /></label>}{error("body")}</div>
    </div>
    <p className="session-notice">修改仅保留在当前页面。导出文件后，放入仓库的 content/repos/ 并提交，网站才会更新。</p>
    <div className="export-actions"><button className="primary-action" type="submit" disabled={blocked}><DownloadSimple size={18} /> 下载资料文件</button><button className="secondary-action" type="button" disabled={blocked} onClick={() => exportFile("copy")}><ClipboardText size={18} /> 复制 Markdown</button><span>{dirty ? "有未导出修改" : "无未导出修改"}</span></div>
