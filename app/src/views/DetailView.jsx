...
import { SourceNotice } from "../components/SourceNotice.jsx";
...
export function DetailView({repo,query="",repositories,onBack,onOpenRepo,onOpenCuration}) {
  ...
  return <main className="detail-page"><button className="back-link" type="button" onClick={onBack}><ArrowLeft size={17}/>返回浏览</button>
    ...
    <div className="guide-layout"><article className="guide-story"><header className="guide-heading"><span>仓库解读</span>{repo.hasGuide&&<small>公开资料整理 · 核查于 {repo.reviewedAt}</small>}</header>
      ...
      {repo.hasGuide?<MarkdownContent key={repo.repoId} query={query}>{repo.contentMarkdown}</MarkdownContent>:<div className="guide-empty"><h2>详细解读尚未补充</h2><p>{repo.summary||repo.description}</p><p>可以先查看上游说明与文档。</p><a href={repo.url} target="_blank" rel="noreferrer">阅读上游 README <ArrowUpRight size={16}/></a></div>}
      ...
      {related.length>0&&<section className="related-feature"><h2>关联仓库</h2>{related.map(r=><RepoLink key={r.repoId} repoId={r.repoId} onOpen={onOpenRepo}><img src={r.avatar} alt=""/><div><strong>{r.name}</strong><small>{repo.relatedNotes?.[r.repoId]||r.summary||r.description}</small></div><ArrowUpRight size={16}/></RepoLink>)}</section>}
    </aside></div>
  </main>;
}
